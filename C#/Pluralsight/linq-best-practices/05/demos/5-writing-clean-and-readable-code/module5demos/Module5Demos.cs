...
{
    // clip 3
    public void SortByAgeLinqChallengePeriklis()
    {
        ...
        "Fraser Forster, 17/03/1988; Kyle Walker-Peters, 13/04/1997; Jan Bednarek, 12/04/1996; James Ward-Prowse, 01/11/1994; Shane Long, 22/01/1987; Mohamed Elyounoussi, 04/08/1994"
            ...
            {
                return new { Name = personDetails.Split(", ")[0].Trim(), DateOfBirth = DateTime.Parse(personDetails.Split(", ")[1]) };
            })
            .Select(p =>
            {
                return new { p.Name, YearsOld = (new DateTime(1, 1, 1) + (DateTime.Now - p.DateOfBirth)).Year - 1 };
            })
            .OrderBy(p => p.YearsOld)
            .Select(p => $"{p.Name}: {p.YearsOld}")
            .Dump();
    }

    // clip 3
    public void SortByAgeLinqChallengePeriklisV2()
    {
        // The following list of football players contains a name followed by a date of birth in day/month/year format.
        // Sort them into a sequence ordered from youngest to oldest, and display the player's name and current age
        "Fraser Forster, 17/03/1988; Kyle Walker-Peters, 13/04/1997; Jan Bednarek, 12/04/1996; James Ward-Prowse, 01/11/1994; Shane Long, 22/01/1987; Mohamed Elyounoussi, 04/08/1994"
            .Split(";")
            .Select(s => s.Split(","))
            .Select(person => new { Name = person[0].Trim(), YearsOld = GetAge(person[1]) })
            .OrderBy(p => p.YearsOld)
            .Select(p => $"{p.Name}: {p.YearsOld}")
            .Dump();
    }
    ...
    {
        return (new DateTime(1, 1, 1) + (DateTime.Now - DateTime.Parse(dateOfBirth))).Year - 1;
    }
    ...
    public void SortByAgeLinqChallenge()
    {
        ...
        {
            Name = s[0].Trim(),
            DateOfBirth = DateTime.ParseExact(s[1].Trim(), "d/M/yyyy", CultureInfo.InvariantCulture)
        })
    ...
    // clip 8
    public void LongestBookVariousSolutionsPer()
    {
        var books = new[] {
